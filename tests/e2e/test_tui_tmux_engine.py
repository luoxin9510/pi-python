"""tmux e2e for the pi engine (``pipython --engine=pi``) — Task 17.

Rewritten copies of the 5 existing legacy-engine scenarios (stream, tool
line, markdown re-render, /tree, byte-path Ctrl+C interrupt — see
``test_tui_tmux.py``, which stays byte-for-byte untouched until Task 18)
plus the 3 new scenarios from spec §7.4: autocomplete overlay appears and
selection writes back; **scrollback preservation** (three turns, then
``capture-pane -S -`` must still contain the first turn's user echo — the
soul assertion of this port); multiline submit composed with Ctrl+J.

Engine-specific notes (vs the legacy file):

- Ready probe: the pi engine prints no ``pipython · <model>`` banner — its
  first frame is the editor's border row (``"─" * width``), so readiness is
...
- Ctrl+C is asserted through the REAL byte path (Task 16 C1): raw mode
  ...
  exercising ``app2._on_stdin_frame``'s byte branch, not a signal.
...
def _start_pane(agent_cwd: Path, fixture: Path) -> TmuxPane:
    ...
    p.start(
        f"uv run --extra tui pipython --engine=pi --model fake/model "
        f"--cwd {shlex.quote(str(agent_cwd))}",
...
# =============================================================================
# 既有 5 场景的 --engine=pi 重写副本（stream / tool line / markdown / tree /
