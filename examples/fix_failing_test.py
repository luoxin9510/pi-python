...
Model override: PI_PYTHON_MODEL=deepseek/deepseek-chat (litellm model id; key via matching env var)
"""
...
MODEL = os.environ.get("PI_PYTHON_MODEL", "anthropic/claude-sonnet-5")


async def main() -> None:
    workdir = Path(tempfile.mkdtemp(prefix="pi-python-demo-")) / "repo"
    shutil.copytree(SAMPLE, workdir)
    session = await create_agent_session(AgentSessionConfig(model=MODEL, cwd=workdir))
    task = (
