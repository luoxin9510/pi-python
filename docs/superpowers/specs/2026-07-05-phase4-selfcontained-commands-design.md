...
用 `/` 拼、macOS 上 `alt`→`Option`（对齐上游 `formatKeyText`/`keyDisplayText`）。app
级键 `Ctrl+O`（工具展开）/`Ctrl+C`/`Ctrl+D` **不在 keybindings 表里**，是 `app.py`
`_on_stdin_frame` 里的裸字节判断——单独硬编码进帮助文案即可。收窄理由：上游 "Other"
